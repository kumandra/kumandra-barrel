...
func DecodeToKumandraPub(address string) ([]byte, error) {
	err := VerityAddress(address, ChainKumandraTestPrefix)
	if err != nil {
		return nil, errors.New("Invalid addrss")
	}
	data := base58.Decode(address)
	if len(data) != (34 + len(ChainKumandraTestPrefix)) {
		return nil, errors.New("base58 decode error")
	}
	return data[len(ChainKumandraTestPrefix) : len(data)-2], nil
}
...
func EncodeToKumandraAddr(publicKeyHash []byte) (string, error) {
	...
	payload := appendBytes(ChainKumandraTestPrefix, publicKeyHash)
	input := appendBytes(SSPrefix, payload)
	ck := blake2b.Sum512(input)
	checkum := ck[:2]
	address := base58.Encode(appendBytes(payload, checkum))
